...
生成処理は階層的に行います。最初にリポジトリのファイル構成、README、一般的なプロジェクトマニフェストからプロジェクト概要を作成します。次に、関連するパッケージまたはプロジェクト領域ごとに完全な差分を逐次分析します。lockファイル、自動生成物、バイナリ、シリアライズされたUnityのSceneやアセットは、その大きな内容自体ではなく、パス、変更統計、コミット、マニフェスト、関連ソースの変更から意味を判断します。最後に各分析結果を統合してリリースノートを生成します。
