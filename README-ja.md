...
通常経路のモデル呼び出しは2段階です。最初に、すべての完全なテキスト差分を、コミット、変更統計、ルートのREADMEやマニフェスト、変更された領域に直接関係するコンテキストファイルと一緒に読み、リリースノート候補を抽出します。独立したプロジェクト概要は生成しません。次に候補を最終Markdownへ整理し、`template-file`があれば同じ呼び出しでテンプレートへ直接記述します。モデルによる統合、検証、自己レビューは行わず、生成結果は人間が確認・編集する下書きとして扱います。

Action独自の文字数制限やコンテキストサイズ制限は設けません。まず関連する証拠全体を1回の分析リクエストでOllamaへ渡し、Ollamaがコンテキスト、トークン、メモリ容量のエラーを返した場合に限って同じ証拠を分割して再試行します。差分内容は破棄せず、分割して得た候補は追加の統合処理を行わず最終生成へ渡します。
