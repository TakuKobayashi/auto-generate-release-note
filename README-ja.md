...
通常経路のモデル呼び出しは2段階です。まず、すべてのdiff hunkについて、所属範囲、変更された宣言、公開シンボル、設定キー、オプション、route、test、import、サイズを含む軽量な索引をローカルで作成します。モデルは完全な索引から、リリースへの影響を理解するために全文が必要なhunkだけを選択します。最終生成には、完全な索引、選択されたhunkの全文、直接関係するプロジェクト情報の要約を渡し、`template-file`があればテンプレート全文もそのまま渡します。コミットメッセージやパスは有用ではあるものの不正確な可能性がある証拠として扱います。モデルによる統合、検証、自己レビューは行わず、生成結果は人間が確認・編集する下書きとして扱います。

文字数やコンテキストサイズを理由にdiffを破棄しません。すべてのhunkはローカルの証拠索引に残ります。証拠選択リクエストがOllamaの物理容量を超えた場合は完全な索引を分割して全パートを処理します。選択されたhunkの全文が最終生成時の容量を超えた場合だけ、完全なパートごとに事実を抽出してから最終生成を1回再試行します。未選択のソースを送らない理由は索引情報で十分と判断されたためであり、文字数の閾値を超えたためではありません。
