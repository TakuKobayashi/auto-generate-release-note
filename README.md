...
The action uses two model stages in the normal path. It first builds a compact local index for every changed diff hunk, including its scope, changed declarations, public symbols, configuration keys, options, routes, tests, imports, and size. The model uses that complete index to select only the hunks whose full source is needed to understand release impact. The final writer receives the complete index, the selected full hunks, compact directly relevant project context, and the full template when `template-file` is set. Commit messages and paths are treated as useful but potentially inaccurate evidence rather than authoritative descriptions. The action does not run model-based consolidation, validation, or self-review passes; the generated text is intended for human editing.

The action does not discard diffs based on a character or context-window limit. Every hunk remains available in the local evidence index. If an evidence-selection request exceeds Ollama's physical capacity, the complete index is partitioned and all partitions are processed. If the selected full hunks exceed capacity during final writing, they are analyzed in complete parts before one final retry. Unselected source is omitted because its indexed evidence was sufficient, not because it crossed a size threshold.
